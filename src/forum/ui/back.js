...
export default function Discussion({ scrolledToBottom, threadID, user }) {
    ...
    return <StyledDiscussion  >
        ...
        {isLoading || isLoading2 ? <>loading... <LinearProgress /></> : <>
            <ThreadUi {...{user,mutators, threadID,Thread: data.Thread, discussion: data, frontendAddReply }} />
            <div >{
...
function ThreadUi({user,mutators,threadID, isReply, discussion, frontendAddReply }) {
    ...
    return <StyledCollection>
        {posts.map((post, i) => <Post {...{user,mutators,threadID, frontendAddReply, i, post, isReply, last: i == posts.length - 1 }} />)}
    </StyledCollection>
}
function Post({mutators,threadID, post, last, isReply, i, frontendAddReply ,user }) {
    ...
    // const {deletePostMutation} = mutators
    // const deletePostMutation =  useMutation(...deletePostMutator(queryClient))
    function frontendDeletePost() {
     ...
     function frontendEditPost(content) {
    ...
    return <StyledPost key={i} last={last}>
        ...
        :<><button onClick={frontendEditPost(newContent)}>save</button></>}
        {<Actions {...{startEditing,newContent,user,mutators,frontendDeletePost,frontendEditPost, replies, sendReply }} />}
    </StyledPost>
}
function Actions({startEditing,newContent,user,mutators,frontendDeletePost, frontendEditPost,replies, sendReply }) {
    ...
    return <div>
        <div className="actions">
            ...
            <button onClick = {()=>{startEditing()}}><Icon >edit</Icon></button>
           <button style={{background:'none',border:'none',color:'#999'}} onClick = {frontendDeletePost}><Icon>delete_outlined</Icon></button>
        </div>
        {formOpen ? replyForm : ""}
        {replies?.Thread ?
            open ? <div>replies:
                <ThreadUi {...{user,mutators, Thread: replies, discussion: replies, isReply: true }} />
